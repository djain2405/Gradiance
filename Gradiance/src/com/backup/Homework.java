...
class Homework {
	...
	public Homework(String courseid) throws IOException {
	...
	private void Reports() {
		...
		System.out.println("3. Find students who scored the maximum score on the first attempt for any homework.");
		...
		try {
			switch(choice) {
				case 1: System.out.println("Enter the ID of homework-1:");
						...
						query = "SELECT NAME, USERID FROM USERS WHERE USERID IN (SELECT USERID FROM USERS WHERE USERLEVEL = 'Undergrad' or USERLEVEL = 'Grad' MINUS" +
							 "(SELECT A.USERID FROM ATTEMPT A WHERE A.EXERCISE_ID = '" + hw_id + "' AND A.COURSE_ID = '" + courseid + "'))";
				...
				case 2: System.out.println("Enter the ID of homework-1:");
						hw_id = reader.nextLine();
						query = "SELECT NAME, USERID FROM USERS WHERE USERID IN" + 
								"(SELECT USERID FROM ATTEMPT WHERE COURSE_ID = '" + courseid + "' AND ATTEMPT_NO = 1 and EXERCISE_ID = '301' AND MARKS = " +
								"(SELECT POINTS_CORRECT*NUM_QUESTIONS FROM ASSESSMENT WHERE COURSE_ID = '" + courseid + "' AND EXERCISE_ID = '" + hw_id + "'))";
						rs = stmt.executeQuery(query);
						while(rs.next()) {
				     		 System.out.println(rs.getString("NAME"));
				     		 System.out.println(rs.getString("USERID"));
				...
				case 3: query = "SELECT NAME, USERID FROM USERS WHERE USERID IN" + 
								"(SELECT A.USERID, A.EXERCISE_ID FROM ATTEMPT A WHERE A.COURSE_ID = '" + courseid + "' AND A.ATTEMPT_NO = 1 AND A.MARKS = " +
								"(SELECT B.POINTS_CORRECT*B.NUM_QUESTIONS FROM ASSESSMENT B WHERE B.COURSE_ID = '" + courseid + "' AND B.EXERCISE_ID = A.EXERCISE_ID))";
						rs = stmt.executeQuery(query);
						while(rs.next()) {
							System.out.println(rs.getString("NAME"));
							System.out.println(rs.getString("USERID"));
				...
				case 4: query = "SELECT USERID, SUM(MARKS) AS TOTAL, AVG(MARKS) AS AVG " +
								"FROM ATTEMPT " +
						...
						System.out.println("=================================================");
						while(rs.next()) {
							System.out.println(rs.getString("USERID") + "\t" + rs.getInt("TOTAL") + "\t" + rs.getFloat("AVG"));
						}
				    
				case 5: query = "SELECT EXERCISE_ID, COUNT(*)/COUNT(DISTINCT USERID) AS AVERAGE " +
								"FROM ATTEMPT " +
								"WHERE COURSE_ID = '" + courseid + "' " +
						...
						System.out.println("=================================================");
						while(rs.next()) {
							System.out.println(rs.getString("EXERCISE_ID") + "\t" + rs.getFloat("AVERAGE"));
						}
					...
					while(rs.next()) {
						if(counter == option) {
							...
							if(retries > 0)
								...
								System.out.println("Number of attempts allowed = infinite");
	...
	private void AddHomework() {
		...
		System.out.println("Enter number of attempts. Enter 0 for infinite attempts:");
		...
		System.out.println("Enter difficulty range. Valid range is 1-5. (Example: 1-3, 1, 5, 1-5... Invalid: 0, 1-8, 6):");
		...
		do {
			...
			if(level <= 0 || level > 5 || levelLow <= 0 || levelLow > 5 || levelHigh <= 0 || levelHigh > 5) {
		...
		try {
			...
			if(difficulty.contains("-"))
				sql = "SELECT Q1.QUESTION_ID, Q1.TEXT FROM QUESTIONS Q1 WHERE Q1.DIFFICULTY >= " + levelLow +
					     " AND Q1.DIFFICULTY <= " + levelHigh + " AND Q1.CHAPTER_ID IN (" +
					                         "SELECT C1.CHAPTER_ID FROM CHAPTERS C1, COURSETEXTBOOK T1" + 
					                         " WHERE T1.COURSE_ID = '" + courseid + "'" +
					                         " AND T1.ISBN = C1.ISBN" +
				...
				sql = "SELECT Q1.QUESTION_ID, Q1.TEXT FROM QUESTIONS Q1 WHERE Q1.DIFFICULTY = " + level +
			     " AND Q1.CHAPTER_ID IN (" + "SELECT C1.CHAPTER_ID FROM CHAPTERS C1, COURSETEXTBOOK T1" + 
			                         " WHERE T1.COURSE_ID = '" + courseid + "'" +
			                         " AND T1.ISBN = C1.ISBN" +
			                         " AND C1.CHAPTER_NAME = '" + topic + "')";
